...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!event) {
    return {
      title: "Event Not Found | COREX 2026",
      description: "The requested event could not be found.",
    };
  }

  return {
    title: `${event.name} | COREX 2026`,
    ...
    openGraph: {
      ...
      images: [
        {
          url: `https://www.bvmtechfest.com${event.image}`,
          width: 1200,
          height: 630,
          alt: event.name,
        },
    ...
    twitter: {
      ...
      title: `${event.name} | COREX 2026`,
      description: event.shortDescription,
      images: [`https://www.bvmtechfest.com${event.image}`],
    },
  };
}
...
export default async function EventPage({ params }: PageProps) {
  ...
  const eventStructuredData = {
    ...
    startDate: event.dates[0], // Assuming first date
    endDate: event.dates[event.dates.length - 1], // Assuming last date
    location: {
      "@type": "Place",
      name: event.venue,
      address: {
        "@type": "PostalAddress",
        addressLocality: "BVM Holy Cross College",
